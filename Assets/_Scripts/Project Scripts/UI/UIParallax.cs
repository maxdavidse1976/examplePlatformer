...
public class UIParallax : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float posX = Mathf.Lerp(transform.position.x, _startPosition.x + (mousePos.x * _parallaxStrength), 5f * Time.deltaTime);
        float posY = Mathf.Lerp(transform.position.y, _startPosition.y + (mousePos.y * _parallaxStrength), 5f * Time.deltaTime);
